...
class GeDiGeneration(GPT2Generation): 
    ...
    def generate(self,
        ...
        with torch.no_grad():
            for step in range(max_len):
                ...
                gedi_logits = torch.stack([expert_logits.log_softmax(-1), antiexpert_logits.log_softmax(-1)], -1).softmax(-1)[:, :, :, 0]#.squeeze(-1)
                # print(gedi_logits.shape)

                # DExperts
                alpha = torch.tensor(alpha).to(self.device)
                ensemble_logits = base_logits + alpha * gedi_logits
